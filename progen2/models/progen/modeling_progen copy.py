...
class ProGenModel(ProGenPreTrainedModel):
    ...
    def forward(
        ...
        if input_ids is not None and inputs_embeds is not None:
            ...
            assert isinstance(input_ids, torch.Tensor), "input_ids should be of type torch.Tensor"
            input_shape = input_ids.size()
            input_ids = input_ids.view(-1, input_shape[-1])
            batch_size = input_ids.shape[0]
            device = input_ids.device
        elif inputs_embeds is not None:
            assert isinstance(inputs_embeds, torch.Tensor), "inputs_embeds should be of type torch.Tensor"
        ...
        # head_mask has shape n_layer x batch x num_attention_heads x N x N# Use getattr to safely handle missing legacy methods
        # self.get_head_mask = getattr(self, "get_head_mask", None)
        # assert self.get_head_mask is None, "The class should either have a get_head_mask function or the property should be set to None"
        head_mask = [None] * self.config.n_layer  # TODO: implement head mask if needed
